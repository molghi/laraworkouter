<div style="flex: 1" class="">
    {{-- title --}}
    <h2 class="text-2xl font-semibold text-snow-400 drop-shadow-[0_0_2px_rgba(255,255,255,0.8)] text-center mb-5">
        Recent Workouts
        {{ !empty(session('finished_workouts')) && count(session('finished_workouts')) > 0 ? '(' . count(session('finished_workouts')) . ')' : '' }}
    </h2>
    ...
    @if (!empty(session('finished_workouts')) && count(session('finished_workouts')) > 0)
        <div class="workout-box max-h-[70vh] overflow-y-scroll p-2">
            @foreach (session('finished_workouts') as $workout)
                <div class="border rounded-md p-4 mb-6 bg-black/50 flex flex-col gap-3">
                    {{-- name --}}
