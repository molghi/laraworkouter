...
<div class="max-w-3xl mx-auto">
  <form action="{{ route('workout.preview') }}" method="POST" class="bg-gray-800/70 text-white px-4 py-6 rounded-lg flex flex-col gap-6">
    @csrf 
            ...
            <div class="{{ $key === 'textarea' ? 'col-span-4' : 'col-span-2' }} flex flex-col gap-1">
                ...
                @if ($key === 'textarea')
                    <textarea required 
                        ...
                        placeholder="{{ $field[2] }}" 
                        autofocus
                        class="w-full min-h-[150px] max-h-[400px] p-2 rounded bg-[#111] transition focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:bg-gray-900">{{ $old_form_data[$field[0]] }}</textarea>
                @else
                    <input required 
                        name="{{ $field[0] }}" 
                        id="{{ $field[0] }}" 
